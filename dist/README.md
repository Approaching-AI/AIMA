...
|------|----------------|------|-------|
| `dist/aima-windows-amd64-v0.5-dev-amd-strix-halo-20260709.exe` | `v0.5-dev-amd-strix-halo-20260709` | 2026-07-09 | **latest** — adds catalog overlay diagnostics for packaged/global default config rollout: `catalog.effective`, `catalog.diff`, `catalog.validate-patch`, stricter patch schema validation, and `deploy.run` failure code + cleanup for newly-created failed deployments. Verified on AMD Ryzen AI Max+ 395 / Radeon 8060S Windows 11 test machine (`cc9b54c`): version, HAL, engine/model/deploy list, catalog status, validate-patch, effective, and diff all exit 0. `serve.bat` uses this one. |
| `dist/aima-windows-amd64-v0.5-dev-amd-strix-halo-20260629d.exe` | `v0.5-dev-amd-strix-halo-20260629d` | 2026-06-29 | makes **`aima deploy` of Qwen3.6-35B work on b9196 too** (the partner's engine), without an engine upgrade or disabling the cache. Root cause: the deploy warmup sent **two identical `Hello` requests**; the 2nd hits a 100%-cached prompt → llama.cpp drops the last token via a partial seq_rm → ABORTs on b9180/b9196. Fix: the **warmup requests now send `cache_prompt:false`** so they don't reuse the slot cache; real serving still caches (full speed). Verified on the partner's exact b9196 (2× identical Hello CRASHES; with `cache_prompt:false` SURVIVES). Kept for rollback. |
...
| `dist/aima-windows-amd64-v0.5-dev-amd-strix-halo.exe` | latest dated build alias | 2026-07-09 | rolling alias for the latest dated Strix Halo build; currently identical to `dist/aima-windows-amd64-v0.5-dev-amd-strix-halo-20260709.exe`. |

The dated file name matches the exe's own `aima version` string. Launcher: `dist/serve.bat`
