...
func Evaluate(intent Intent, observation Observation, now time.Time) Decision {
	...
	restartDelta := 0
	if containerRuntime && observation.Restarts > next.ObservedRestartCount {
	...
	if restartDelta > 0 && next.AttemptCount >= next.Policy.MaxAttempts {
		next.RecoveryState = StateQuarantined
		next.NextAttemptAt = time.Time{}
		return Decision{Intent: next, Action: ActionQuarantine}
	}
