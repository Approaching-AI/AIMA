...
# 测试 scan
ssh user@<REDACTED_IP> 'echo <base64pwd> | base64 -d | sudo -S ~/aima engine scan 2>&1'

# 验证 list
ssh user@<REDACTED_IP> '~/aima engine list'

# 检查 Pod YAML 中的关键字段
ssh user@<REDACTED_IP> 'sudo kubectl get pod <name> -o yaml | grep -A2 "LD_LIBRARY_PATH\|hostPath\|image:"'
```
