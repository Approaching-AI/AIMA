...
func (c *Controller) runOnce(ctx context.Context, respectInterval bool) error {
	...
	for _, stored := range intents {
		...
		if stored == nil || stored.DesiredState != DesiredRunning {
			continue
		}
		if stored.RecoveryState == StateQuarantined {
			if quarantineDeleteDue(*stored, now) {
				if err := c.enforceQuarantineDelete(ctx, *stored); err != nil {
					wrapped := fmt.Errorf("enforce deployment quarantine %s: %w", stored.Name, err)
					slog.Warn("deployment quarantine enforcement failed", "deployment", stored.Name, "error", err)
...
func (c *Controller) reconcileIntent(ctx context.Context, intent Intent) error {
	...
	case ActionQuarantine:
		...
		return c.enforceQuarantineDelete(ctx, committed)
	}
	return nil
}
...
func (c *Controller) enforceQuarantineDelete(ctx context.Context, committed Intent) error {
	if c.delete == nil {
		return c.recordQuarantineDeleteFailure(ctx, committed, fmt.Errorf("delete function is unavailable"), c.now().UTC())
	}
	claimCtx := WithReconcilerClaim(ctx, committed)
	if err := c.delete(claimCtx, cloneIntent(committed)); err != nil {
		return c.recordQuarantineDeleteFailure(ctx, committed, err, c.now().UTC())
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	completed := cloneIntent(committed)
	completed.NextAttemptAt = time.Time{}
	persisted, ok, err := c.commit(ctx, completed, committed.Revision)
	if err != nil {
		return err
	}
	if ok {
		c.emitAudit(ctx, auditEvent(persisted, AuditQuarantineEnforced, AuditResultEnforced, "runtime object deleted"))
	}
	return nil
}

func (c *Controller) recordQuarantineDeleteFailure(ctx context.Context, committed Intent, deleteErr error, now time.Time) error {
	failure := cloneIntent(committed)
	failure.LastError = SanitizeText(deleteErr.Error())
	failure.NextAttemptAt = now.Add(recoveryBackoff(failure.Policy, failure.AttemptCount))
	persisted, ok, err := c.commit(ctx, failure, committed.Revision)
	if err != nil {
...
func quarantineDeleteDue(intent Intent, now time.Time) bool {
	return (intent.Runtime == "docker" || intent.Runtime == "k3s") &&
		!intent.NextAttemptAt.IsZero() && attemptDue(intent.NextAttemptAt, now)
}
