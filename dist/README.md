...
|------|----------------|------|-------|
| `dist/aima-windows-amd64-v0.5-dev-amd-strix-halo-20260629c.exe` | `v0.5-dev-amd-strix-halo-20260629c` | 2026-06-29 | **latest** — Qwen3.6-35B deploy crash root cause **corrected: it's the llama.cpp ENGINE VERSION** (same-machine A/B: b9196 CRASHES, b9330 SURVIVES, identical command). Removed the `cache_prompt:false` catalog workaround (it disabled the prompt cache = slower); **real fix = run engine b9330+** (AIMA auto-downloads it; pin `AIMA_ENGINE_DIR` to b9330+). Keeps the OpenClaw 撤回同步 (exclude/include) feature. `serve.bat` uses this one. |
