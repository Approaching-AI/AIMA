...
The registered AIBook product version is `v0.4.1`. The Debian package version appends the build date so builds remain identifiable, while the application name, package name, and process name remain `AIMA`, `aima-aibook`, and `aima`. The package installs the orange AIMA icon registered for this edition.
