...
func phaseUserPrompt(phase string) string {
	...
	case "check":
		return "开始 check 阶段。读取实验结果与 summary.md，记录发现、bugs、设计疑虑，并把 Confirmed Blockers / Do Not Retry This Cycle / Evidence Ledger 写成结构化内容；Recommended Configurations 里只有真实 benchmark 证据足够时才可写 validated，否则写 tuned 或 provisional；用 done(verdict) 结束。"
	case "act":
