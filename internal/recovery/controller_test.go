...
func TestControllerQuarantinesContainerBeforeDelete(t *testing.T) {
	...
	if len(commits) != 1 || commits[0].RecoveryState != StateQuarantined || commits[0].DesiredState != DesiredRunning {
		t.Fatalf("committed intent = %+v, want desired running and quarantined", commits)
	}
	if !reflect.DeepEqual(events, []string{"cas", "delete"}) {
		t.Fatalf("events = %v, want quarantine CAS before delete", events)
	}
